...
public class MediaCodecVideoRendererWithWorkarounds extends MediaCodecVideoRenderer {
    ...
    private boolean isDolbyVisionProfile7WorkaroundRequired(Format format) throws MediaCodecUtil.DecoderQueryException {
        if(!MimeTypes.VIDEO_DOLBY_VISION.equals(format.sampleMimeType)) {
            return false;
        }

        boolean isSupported = isDvProfileLevelSupported(android.media.MediaCodecInfo.CodecProfileLevel.DolbyVisionProfileDvheDtb);
        if(isSupported) {
            return false;
        }

        Pair<Integer, Integer> profileAndLevel = CodecSpecificDataUtil.getCodecProfileAndLevel(format);
        return profileAndLevel != null && profileAndLevel.first == android.media.MediaCodecInfo.CodecProfileLevel.DolbyVisionProfileDvheDtb;
    }

    private static boolean isDvProfileLevelSupported(int checkedProfileLevel) throws MediaCodecUtil.DecoderQueryException {
        List<MediaCodecInfo> dvDecoders = MediaCodecUtil.getDecoderInfos(
                MimeTypes.VIDEO_DOLBY_VISION,
                false /* requiresSecure */,
                false /* requiresTunneling */);

        return dvDecoders.stream().anyMatch(decoder -> {
            android.media.MediaCodecInfo.CodecProfileLevel[] profileLevels = decoder.getProfileLevels();
            return Arrays.stream(profileLevels)
                    .anyMatch(profileLevel ->
                            profileLevel.level == checkedProfileLevel);
        });
    }
}
