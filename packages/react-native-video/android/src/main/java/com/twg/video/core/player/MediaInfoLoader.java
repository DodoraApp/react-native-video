...
public class MediaInfoLoader {
  ...
  private static final Pattern LIVE_EXT_PATTERN =
      Pattern.compile("\\.(m3u8?|mpd)\\b", Pattern.CASE_INSENSITIVE);
  ...
  public static MediaInfo loadMediaInfo(String streamUrl) {
    ...
    if (LIVE_EXT_PATTERN.matcher(streamUrl).find()) {
      Log.d(TAG, "Skipping media info extraction for live stream: " + streamUrl);
      return null;
    }
    ...
    try {
      Log.d(TAG, "Starting media info extraction for: " + streamUrl);

      // Build and extract media info using nextlib
      nativeMediaInfo = new MediaInfoBuilder().from(streamUrl).build();

      if (nativeMediaInfo == null) {
        Log.w(TAG, "MediaInfo extraction returned null");
        return null;
      }
      ...
      if (videoStream != null) {
        ...
        Log.w(TAG, "No video stream found in media info");
      }

      // Extract chapters
      List<MediaInfo.Chapter> chapters = new ArrayList<>();
      List<io.github.anilbeesetti.nextlib.mediainfo.Chapter> nativeChapters =
          nativeMediaInfo.getChapters();

      if (nativeChapters != null && !nativeChapters.isEmpty()) {
        Log.d(TAG, "Found " + nativeChapters.size() + " chapters");
        ...
        for (io.github.anilbeesetti.nextlib.mediainfo.Chapter nativeChapter : nativeChapters) {
          ...
          MediaInfo.ChapterType type =
              MediaInfo.parseChapterType(title, startMs, endMs, duration);

          chapters.add(new MediaInfo.Chapter(title, startMs, endMs, type));

          Log.d(TAG, "Chapter: " + title + " (" + startMs + "ms - " + endMs + "ms) Type: " + type);
        }
      }

      MediaInfo result =
          new MediaInfo(duration, videoWidth, videoHeight, frameRate, bitrate, chapters);
      Log.d(TAG, "Media info extraction successful: " + result);

      return result;

    } catch (Exception e) {
      Log.e(TAG, "Failed to extract media info: " + e.getMessage(), e);
      return null;
    } finally {
      ...
      if (nativeMediaInfo != null) {
        try {
          ...
          Log.w(TAG, "Error releasing media info: " + e.getMessage());
        }
      }
    }
  }
