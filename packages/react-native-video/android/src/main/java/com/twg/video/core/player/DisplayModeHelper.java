 ...
 *
 * <p>Sets the display mode to match the video's framerate during playback,
...
public class DisplayModeHelper {
  ...
  public void setActivity(@Nullable Activity activity) {
    if (this.activity != null && activity != this.activity && displayModeChanged) {
      Log.d(TAG, "Activity changed while display mode active, restoring");
  ...
  public void setEnabled(boolean enabled) {
    ...
    Log.d(TAG, "Framerate matching " + (enabled ? "enabled" : "disabled"));
  ...
  public void setDisplayMode(float frameRate, int width, int height) {
    ...
    if (frameRate <= 0 || width <= 0 || height <= 0) {
      Log.w(TAG, "Invalid parameters: " + width + "x" + height + "@" + frameRate);
      return;
    }
  ...
  private void applyDisplayMode(float frameRate, int width, int height) {
    if (!isActivityValid()) {
      Log.w(TAG, "Activity invalid, skipping display mode change");
      return;
    }
    ...
    if (supportedModes == null || supportedModes.length == 0) {
      Log.d(TAG, "No supported display modes");
      return;
    }
    ...
    if (originalMode == null) {
      ...
      Log.d(TAG, "Stored original mode: " + formatMode(originalMode));
    }

    Display.Mode bestMode = findBestMode(supportedModes, frameRate, width, height);
    if (bestMode == null) {
      Log.d(TAG, "No suitable mode for " + width + "x" + height + "@" + frameRate);
      return;
    }

    Display.Mode currentMode = display.getMode();
    if (currentMode != null && currentMode.getModeId() == bestMode.getModeId()) {
      Log.d(TAG, "Already in optimal mode: " + formatMode(bestMode));
      return;
    }

    Log.d(TAG, "Switching to " + formatMode(bestMode) + " for video " + width + "x" + height + "@" + frameRate);
      ...
      displayListener =
          new DisplayManager.DisplayListener() {
            ...
            public void onDisplayChanged(int displayId) {
              Log.d(TAG, "Display changed callback received");
              unregisterDisplayListener();
    ...
    try {
      ...
      mainHandler.postDelayed(
          () -> {
            if (isWaitingForModeSettle) {
              Log.d(TAG, "Mode change settle timeout, completing");
              unregisterDisplayListener();
              onModeChangeComplete(targetMode);
            }
          },
      ...
      Log.e(TAG, "Failed to set display mode: " + e.getMessage(), e);
  ...
  private void onModeChangeComplete(Display.Mode targetMode) {
    isWaitingForModeSettle = false;
    Log.d(TAG, "Display mode change settled");
  ...
  public void restoreOriginalDisplayMode() {
    ...
    if (!isActivityValid()) {
      Log.d(TAG, "Activity invalid, clearing state without restore");
    ...
    try {
      Log.d(TAG, "Restoring original mode: " + formatMode(originalMode));
      WindowManager.LayoutParams params = window.getAttributes();
      params.preferredDisplayModeId = originalMode.getModeId();
      window.setAttributes(params);
    } catch (Exception e) {
      Log.e(TAG, "Failed to restore display mode: " + e.getMessage(), e);
    }
  ...
  @RequiresApi(api = Build.VERSION_CODES.M)
  @Nullable
  private Display.Mode findBestMode(
      Display.Mode[] modes, float videoFrameRate, int videoWidth, int videoHeight) {
